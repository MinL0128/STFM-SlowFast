import sys
sys.path.append("/root/autodl-tmp/Bullying10K-main")
...
sys.path.append("/root/autodl-tmp/Bullying10K_data/")
...
def train_model(dataset=dataset, save_dir=save_dir, num_classes=num_classes, lr=lr,
    ...
    #     model = TimeSformer(img_size=224, num_classes=10, in_chans=2, num_frames=16,
    ...
    if args.opt == "sgd":
        optimizer = optim.SGD(train_params, lr=lr, momentum=0.9, weight_decay=args.weight_decay)
    elif args.opt == "adamw":
        optimizer = optim.AdamW(train_params, lr=lr, weight_decay=args.weight_decay)
    ...
    if dataset == "bullying10k":
        ...
        train_dataloader, test_dataloader, _, _ = get_bullying10k_data(batch_size=args.batch_size, step=args.step,
                                                                       gap=args.gap, size=args.size)
        val_dataloader = train_dataloader
    else:
        train_dataloader = DataLoader(VideoDataset(dataset=dataset, split='train', clip_len=16),
                                      batch_size=args.batch_size, shuffle=True, num_workers=4)
        val_dataloader = DataLoader(VideoDataset(dataset=dataset, split='val', clip_len=16), batch_size=args.batch_size,
                                    num_workers=4)
        test_dataloader = DataLoader(VideoDataset(dataset=dataset, split='test', clip_len=16),
    ...
    for epoch in range(resume_epoch, num_epochs):
        ...
        if useTest and epoch % test_interval == (test_interval - 1):
            ...
            for i in range(10):
                precision, recall, thresholds = precision_recall_curve(y_true_bin[:, i],
                                                                       predall.gather[:, i].cpu().numpy())
                p.append(precision)
                r.append(recall)
                t.append(thresholds)

            np.save("pr.npy", (p, r, t))
            writer.add_pr_curve(tag="pr_curve/val", labels=F.one_hot(yall.gather, 10),
                                predictions=F.softmax(predall.gather, dim=1), global_step=epoch)
